...
void MainMenu::onRedraw(draw_mode_t what) {
  if (what & BACKGROUND) {
    ...
    #define ADVANCED_SETTINGS_POS BTN_POS(1,2), BTN_SIZE(2,1)
      ...
      #define CUSTOM_MENU_MAIN_POS BTN_POS(2,3), BTN_SIZE(1,1)
    #else
    ...
    #define DISABLE_STEPPERS_POS  BTN_POS(4,3), BTN_SIZE(3,1)
      ...
      #define CUSTOM_MENU_MAIN_POS BTN_POS(5,4), BTN_SIZE(2,1)
    #else
  ...
  if (what & FOREGROUND) {
    ...
    cmd.colors(normal_btn)
       ...
       #if ENABLED(CUSTOM_MENU_MAIN)
        .tag(11).button(CUSTOM_MENU_MAIN_POS, GET_TEXT_F(MSG_USER_MENU))
       #endif
       .colors(action_btn)
